...
module.exports = {
    ...
    postWithErrorChecking: function (config, route, data) {
        return function (next) {
            request(config.mock)
                .post(route)
                .set('Cookie', mapCookies(config.res.headers['set-cookie']))
    ...
    get: function (config, route, options) {
        var urlParams = '';
        if (options && options.url_params) {
            urlParams = '?';
            var first = true;
            for (var key in options.url_params) {
                if (!first) {
                    urlParams += '?';
                }
                if (first) {
                ...
                urlParams += key+'='+options.url_params[key];
            }
            //urlParams = '?user_id='+user_id;
        }
        return function (next) {
    ...
    delete: function (config, route, options) {
        var urlParams = '';
        if (options && options.url_params) {
            urlParams = '?';
            var first = true;
